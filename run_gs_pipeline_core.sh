...
if [[ "$is_video" -eq 1 ]]; then
  ...
  echo "[1/10] Extracting high-quality frames with ffmpeg..."
  ...
  echo "[1/10] Copying input images directory..."
  if [[ ! -d "$INPUT" ]]; then
...
echo "[2/10] Running COLMAP feature extraction (quality-first settings)..."
...
run_matching() {
  local mode="$1"
  echo "[3/10] Running COLMAP matching ($mode)..."
...
echo "[4/10] Running sparse mapping..."
mapper_ok=0
...
if [[ "$DO_SPARSE_DENSIFY" -eq 1 ]]; then
  ...
  echo "[5/10] Running sparse point densification (point_triangulator)..."
  ...
  echo "[5/10] Sparse densification skipped (--no-sparse-densify)."
fi
...
echo "[6/10] Converting sparse model to PLY..."
"$COLMAP_BIN" model_converter \
  --input_path "$MODEL_PATH" \
  --output_path "$MODEL_PATH/points3D_sparse.ply" \
  --output_type PLY

echo "[7/10] Exporting COLMAP text model (for gs training tools that require txt)..."
...
if [[ "$DO_DENSE" -eq 1 ]]; then
  if "$COLMAP_BIN" patch_match_stereo -h >/dev/null 2>&1; then
    mkdir -p "$DENSE_DIR"
    echo "[8/10] Running image undistortion + PatchMatch stereo..."
    ...
    echo "[9/10] Fusing dense depth maps to PLY..."
    ...
    echo "[8/10] Dense step requested but skipped: this COLMAP build requires CUDA for patch_match_stereo." >&2
    echo "[9/10] Done (sparse outputs only)."
    DO_DENSE="0"
  fi
else
  echo "[8/10] Dense step skipped (use --dense for densest PLY on CUDA-enabled COLMAP)."
  echo "[9/10] Done."
fi
...
if [[ "$BRUSH_AUTO" -eq 1 ]]; then
  echo
  echo "[10/10] Brush training/export..."
